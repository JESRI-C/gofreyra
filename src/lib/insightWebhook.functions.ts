...
const PayloadSchema = z.object({
  ...
  interest_area: z.string().max(80).optional().nullable(),
 ...
 *   2. Opret Make/Zapier-scenarie med Webhook → Google Sheets (Add Row)
 *   3. Indsæt webhook-URL'en som secret INSIGHT_ACCESS_WEBHOOK_URL
 *
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return { forwarded: false, reason: "http_error" as const };
      }
      return { forwarded: true };
    } catch (err) {
      console.error("[insight-lead] webhook error:", err);
      return { forwarded: false, reason: "network_error" as const };
    }
  });
